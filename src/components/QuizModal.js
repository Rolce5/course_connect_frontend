    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-40 p-4">
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-4 lg:p-6">
          ...
          {quizAttempts?.length > 0 && weakTopics?.length > 0 && (
            <div className="mb-4 p-3 bg-blue-50 rounded flex items-start">
              <FiAlertTriangle className="text-blue-500 mr-2 mt-0.5 flex-shrink-0" />
              <div>
                <p className="font-medium">Focus Areas</p>
                <p className="text-sm">
                  You've previously struggled with: {weakTopics.join(", ")}
                </p>
              </div>
            </div>
          )}
