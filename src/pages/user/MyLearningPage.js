  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-4 md:mb-0">My Learning</h1>
      ...
      {filteredCourses.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((enrollment) => (
            <div key={enrollment.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-medium text-gray-900">{enrollment?.title}</h3>
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                    {enrollment?.category}
                  </span>
                </div>
                <p className="text-sm text-gray-500 mb-4">Instructor: <span className="font-bold capitalize">{enrollment?.instructor.first_name} {enrollment?.instructor.last_name}</span>
</p>
                  ...
                  <>
                    <div className="mb-3">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-gray-700">Progress: {enrollment.progress}%</span>
                        <span className="text-gray-500">Last accessed: {enrollment.lastAccessed}</span>
                      </div>
                ...
                <Link to={`/${enrollment.title}/${enrollment.courseId}/learn`} className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
  {enrollment.progress === undefined || enrollment.progress === null || enrollment.progress === 0 
    ? 'Start Course' 
    : enrollment.status === 'COMPLETED' 
      ? 'View Certificate' 
      : 'Continue Learning'}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <h3 className="mt-2 text-lg font-medium text-gray-900">
            {activeTab === 'in-progress' ? 'No active courses' : 'No completed courses'}
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {recommendedCourses.map((course) => (
            <div key={course.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              ...
              />
              <div className="p-4">
                <h3 className="text-md font-medium text-gray-900 mb-1"> {course.title}</h3>
                <p className="text-sm text-gray-500 mb-3 capitalize">{course.instructor.first_name} {course.instructor.first_name}</p>
                <Button onClick={() => navigate(`/courses/${course.id}`)} className="inline-flex justify-center items-center px-4 !py-2 border border-gray-300 !shadow-sm text-sm !rounded-md text-gray-700 bg-white hover:bg-gray-50">
                  Explore
                </Button>
              </div>
            </div>
